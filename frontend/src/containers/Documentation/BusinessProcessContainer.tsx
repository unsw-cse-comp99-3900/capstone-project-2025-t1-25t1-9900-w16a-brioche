  ...
  return (
    <section id="business-process" className="mb-16">
      ...
      <p className="mt-4 text-lg text-secondary-500">
        Our e-invoicing platform follows a streamlined process to create,
        validate, and send e-invoices.
      </p>
        ...
        <div className="flex flex-col items-center">
          {processSteps.map((step, idx) => (
            <React.Fragment key={step.id}>
              <div
                className={`flex items-center justify-center w-64 h-20 ${step.color} rounded-lg ${step.borderColor} text-center p-4`}
              >
                <div>
                  <div className={`font-medium ${step.textColor}`}>
                    {step.title}
                  </div>
                  <div
                    className={`text-xs mt-1 ${step.textColor.replace("-700", "-600")}`}
                  >
                    {step.subtitle}
                  </div>
                </div>
      ...
      <div className="mt-4 space-y-6">
        {processSteps.map((step) => (
          <div key={step.id}>
            <h4 className={`text-lg font-medium ${step.textColor}`}>
              {step.title}
            </h4>
            {step.description.map((desc, i) => (
              <p key={i} className="mt-2 text-secondary-700">
                {desc}
              </p>
            ))}
            {step.subpoints && (
